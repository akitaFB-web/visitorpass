...
const G = () => (
  <style>{`
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    .inp{
      width:100%;background:var(--s1);border:1.5px solid var(--br);border-radius:8px;
      color:var(--t1);font-family:inherit;font-size:13.5px;padding:10px 13px;
    ...
    .btn{
      ...
      font-family:inherit;font-weight:600;border-radius:8px;transition:all .18s;font-size:13.5px;
    }
    ...
    .tag{
      ...
      background:var(--s3);color:var(--t2);border:1px solid var(--br);
    }
    ...
    .sec-label{
      ...
      color:var(--t3);display:flex;align-items:center;gap:8px;margin-bottom:14px;
    }
    ...
    th{
      background:var(--s2);color:var(--t3);font-size:11px;font-weight:700;
      letter-spacing:.8px;text-transform:uppercase;padding:10px 16px;text-align:left;
    }
    ...
    .tab-btn{
      ...
      font-weight:500;border:1.5px solid transparent;background:transparent;
      color:var(--t3);transition:all .18s;font-family:inherit;white-space:nowrap;
    }
    ...
    .loader{
      display:inline-block;width:16px;height:16px;border:2.5px solid rgba(255,255,255,.2);
    ...
    .panel-box{
      background:var(--s1);border:1px solid var(--br2);border-radius:14px;
      width:100%;max-width:520px;box-shadow:0 24px 64px rgba(0,0,0,.6);
    ...
    .panel-foot{
      padding:16px 24px;border-top:1px solid var(--br);
      display:flex;justify-content:flex-end;gap:10px;
    }
    .toast{
      position:fixed;bottom:24px;right:24px;z-index:999;
      background:var(--s2);border:1px solid var(--br2);
      padding:14px 20px;border-radius:10px;font-size:13.5px;
      box-shadow:var(--sh);max-width:340px;animation:fadeUp .3s ease;
      display:flex;align-items:center;gap:10px;cursor:pointer;
    }
    .toast-ok{border-left:4px solid var(--ag)}
    .toast-er{border-left:4px solid var(--ae)}
    .toast-in{border-left:4px solid var(--ab)}
    ...
    <div style={{ position: "fixed", bottom: 24, right: 24, zIndex: 999, display: "flex", flexDirection: "column", gap: 10 }}>
      {items.map(t => (
        <div key={t.id} className={`toast toast-${t.type}`} onClick={() => remove(t.id)}>
          <span style={{ fontSize: 18 }}>{t.type === "ok" ? "✅" : t.type === "er" ? "❌" : "ℹ️"}</span>
          <div>
            <div>{t.msg}</div>
...
// ─── VIEW 1 · Employee Form ───────────────────────────────────────────────────
function EmployeeForm({ onSubmit }) {
  const blank = {
    visitorName: "", company: "", purpose: "", visitDate: "",
    enterTime: "", exitTime: "",
    requesterName: "", department: "", employeeId: "", supervisorEmail: "",
  };
  ...
  const validate = () => {
    ...
    if (!f.supervisorEmail.trim() || !f.supervisorEmail.includes("@"))
      e.supervisorEmail = "อีเมลไม่ถูกต้อง";
    return e;
  };
  ...
  if (done) return (
    <div className="fu" style={{ maxWidth: 520, margin: "0 auto", textAlign: "center", padding: "60px 20px" }}>
      ...
      <p style={{ color: "var(--t2)", marginBottom: 6 }}>
        รหัสคำร้อง:{" "}
        <span style={{ fontFamily: "'DM Mono',monospace", color: "var(--ac)", fontSize: 16, fontWeight: 600 }}>
          {done.id}
        </span>
      </p>
      <p style={{ color: "var(--t2)", fontSize: 13, marginBottom: 28 }}>
        อีเมลแจ้งหัวหน้าถูกส่งไปที่{" "}
        <strong style={{ color: "var(--t1)" }}>{done.supervisorEmail}</strong> แล้ว
      </p>
    ...
    <div className="fu" style={{ maxWidth: 700, margin: "0 auto" }}>
      <div style={{ textAlign: "center", marginBottom: 32 }}>
        <div style={{
          ...
          background: "linear-gradient(135deg,rgba(0,217,176,.15),rgba(14,165,233,.15))",
          border: "1px solid rgba(0,217,176,.4)",
        }}>
      ...
      <div className="card" style={{ padding: 24, marginBottom: 16 }}>
        ...
        <div className="g2" style={{ marginBottom: 14 }}>
          <Field label="ชื่อ-นามสกุล *" error={err.visitorName}>
            <input className={`inp${err.visitorName ? " err" : ""}`}
              placeholder="ชื่อเต็มของผู้เข้าเยี่ยม"
              value={f.visitorName} onChange={set("visitorName")} />
          </Field>
          <Field label="บริษัท / องค์กร *" error={err.company}>
            <input className={`inp${err.company ? " err" : ""}`}
              placeholder="ชื่อบริษัทหรือหน่วยงาน"
              value={f.company} onChange={set("company")} />
          </Field>
        </div>
        <Field label="วัตถุประสงค์การเข้าพบ *" error={err.purpose}>
          <textarea className={`inp${err.purpose ? " err" : ""}`}
            placeholder="เช่น ประชุมติดตามงาน, ตรวจสอบ, ส่งของ..."
            value={f.purpose} onChange={set("purpose")} />
        </Field>
        <div className="g3" style={{ marginTop: 14 }}>
          <Field label="วันที่เข้าพบ *" error={err.visitDate}>
            <input type="date" className={`inp${err.visitDate ? " err" : ""}`}
              value={f.visitDate} onChange={set("visitDate")} />
          </Field>
          <Field label="เวลาเข้า *" error={err.enterTime}>
            <input type="time" className={`inp${err.enterTime ? " err" : ""}`}
              value={f.enterTime} onChange={set("enterTime")} />
          </Field>
          <Field label="เวลาออก (ประมาณ)">
            <input type="time" className="inp"
              value={f.exitTime} onChange={set("exitTime")} />
          </Field>
        </div>
      </div>
      ...
      <div className="card" style={{ padding: 24, marginBottom: 24 }}>
        ...
        <div className="g2" style={{ marginBottom: 14 }}>
          <Field label="ชื่อ-นามสกุลผู้ร้องขอ *" error={err.requesterName}>
            <input className={`inp${err.requesterName ? " err" : ""}`}
              placeholder="ชื่อพนักงาน"
              value={f.requesterName} onChange={set("requesterName")} />
          </Field>
          <Field label="รหัสพนักงาน *" error={err.employeeId}>
            <input className={`inp${err.employeeId ? " err" : ""}`}
              placeholder="เช่น EMP-0042"
              value={f.employeeId} onChange={set("employeeId")} />
          </Field>
        </div>
        <div className="g2">
          <Field label="แผนก *" error={err.department}>
            <input className={`inp${err.department ? " err" : ""}`}
              placeholder="เช่น ฝ่ายผลิต, QC, Admin"
              value={f.department} onChange={set("department")} />
          </Field>
          <Field label="อีเมลหัวหน้า (เพื่อส่ง Approve) *" error={err.supervisorEmail}>
            <input type="email" className={`inp${err.supervisorEmail ? " err" : ""}`}
              placeholder="supervisor@company.com"
              value={f.supervisorEmail} onChange={set("supervisorEmail")} />
          </Field>
        </div>
      </div>

      <div style={{ display: "flex", justifyContent: "flex-end" }}>
        <button className="btn btn-p" onClick={submit} disabled={busy} style={{ minWidth: 170 }}>
          {busy ? <><div className="loader" /> กำลังส่ง...</> : <>📤 ส่งคำร้องขอ</>}
        </button>
      </div>
    </div>
  );
...
function ManagerView({ visitors, onApprove, onReject }) {
  ...
  const confirm = async () => {
    setBusy(true);
    if (panel.action === "approve") await onApprove(panel.visitor.id, note);
    else await onReject(panel.visitor.id, note);
    setBusy(false);
    closePanel();
  };
        ...
        <div>
          ...
          <p style={{ fontSize: 13, color: "var(--t2)", marginTop: 2 }}>
            {pending.length === 0
              ...
              : `${pending.length} รายการรอการพิจารณา`}
          </p>
        </div>
      </div>
      ...
      {pending.length === 0 ? (
        <div className="card" style={{ padding: 0 }}>
          <div className="empty">
            ...
            <p style={{ fontWeight: 600, fontSize: 15, color: "var(--t2)" }}>ไม่มีรายการรออนุมัติ</p>
            <p style={{ fontSize: 13, marginTop: 5 }}>รายการทั้งหมดได้รับการพิจารณาแล้ว</p>
          </div>
        </div>
      ) : (
            ...
            <div key={v.id} className="card fu" style={{ padding: 22, animationDelay: `${i * 55}ms` }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
                <div style={{ display: "flex", gap: 12, alignItems: "flex-start", flex: 1 }}>
                  <div style={{
                    width: 44, height: 44, borderRadius: 10, background: "var(--s2)",
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 6 }}>
                      ...
                      <span className="badge" style={{ color: STATUS.pending.color, background: STATUS.pending.bg }}>
                        {STATUS.pending.label}
                      </span>
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(160px,1fr))", gap: "8px 18px", marginTop: 10 }}>
                      {[
                        ["🎯", "วัตถุประสงค์", v.purpose],
                        ["📅", "วันที่", v.visitDate],
                        ["🕐", "เวลา", `${v.enterTime}${v.exitTime ? " – " + v.exitTime : ""}`],
                        ["👷", "ผู้ร้องขอ", `${v.requesterName} (${v.employeeId})`],
                        ["🏢", "แผนก", v.department],
                        ["📨", "ส่งเมื่อ", thaiDate(v.submittedAt)],
                      ].map(([ic, lbl, val]) => (
                        <div key={lbl}>
                          <div style={{ fontSize: 10.5, color: "var(--t3)", marginBottom: 2 }}>{ic} {lbl}</div>
        ...
        <div className="panel" onClick={e => e.target === e.currentTarget && closePanel()}>
          <div className="panel-box">
            <div className="panel-head">
              <div>
                <div style={{ fontWeight: 700, fontSize: 16 }}>
                  {panel.action === "approve" ? "✅ ยืนยันการอนุมัติ" : "❌ ยืนยันการปฏิเสธ"}
                </div>
                <div style={{ fontSize: 12, color: "var(--t2)", marginTop: 3 }}>
                  {panel.visitor.visitorName} — {panel.visitor.company}
                </div>
              </div>
            ...
            <div className="panel-foot">
              ...
              <button
                className={`btn ${panel.action === "approve" ? "btn-ok" : "btn-ng"}`}
                onClick={confirm} disabled={busy}
              >
                {busy
                  ? <><div className="loader" /> กำลังบันทึก...</>
                  : panel.action === "approve" ? "✅ ยืนยันอนุมัติ" : "❌ ยืนยันปฏิเสธ"}
              </button>
            </div>
          </div>
        </div>
...
function AdminDashboard({ visitors }) {
  ...
  const list = visitors.filter(v => {
    ...
    return !q || [v.visitorName, v.company, v.department, v.requesterName]
...
${todayApproved.length === 0
    ? "ไม่มีบุคคลภายนอกที่ได้รับอนุมัติให้เข้าพบในวันนี้"
    : todayApproved.map((v, i) =>
`${i + 1}. ${v.visitorName} (${v.company})
  ...
  return (
    <div className="fu" style={{ maxWidth: 1040, margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 14, marginBottom: 28 }}>
        ...
        <button className="btn btn-p" onClick={() => setShowEmail(!showEmail)} style={{ fontSize: 13 }}>
          📧 ตัวอย่างอีเมลเช้า
        </button>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 24 }}>
        {[
          { n: stats.total,    l: "ทั้งหมด",     c: "var(--ab)" },
          { n: stats.pending,  l: "รออนุมัติ",   c: "var(--aw)" },
          { n: stats.approved, l: "อนุมัติแล้ว", c: "var(--ag)" },
          { n: stats.rejected, l: "ปฏิเสธ",      c: "var(--ae)" },
        ].map(s => (
      ...
      {showEmail && (
        <div className="card fi" style={{ padding: 20, marginBottom: 20, borderColor: "rgba(14,165,233,.35)" }}>
          ...
          <pre style={{
            ...
            padding: 16, fontSize: 12.5, color: "var(--t2)", whiteSpace: "pre-wrap",
            fontFamily: "'DM Mono',monospace", lineHeight: 1.75,
          }}>{emailPreview}</pre>
          <p style={{ fontSize: 11.5, color: "var(--t3)", marginTop: 10 }}>
            💡 เชื่อม Google Apps Script ให้ส่งอัตโนมัติทุก 7:00 น. ได้ตามคู่มือ
          </p>
        </div>
      )}

      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", marginBottom: 14 }}>
        <input className="inp" placeholder="🔍 ค้นหา ชื่อ / บริษัท / แผนก..."
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {[["all","ทั้งหมด"],["pending","รออนุมัติ"],["approved","อนุมัติ"],["rejected","ปฏิเสธ"]].map(([k, l]) => (
            <button key={k} className={`tab-btn${filter === k ? " on" : ""}`}
              onClick={() => setFilter(k)} style={{ fontSize: 12, padding: "7px 14px" }}>
              {l}
            </button>
          ))}
        </div>
              ...
              <tbody>
                {list.map(v => (
                  <tr key={v.id}>
                    ...
                    <td>
                      <span className="badge" style={{ color: STATUS[v.status].color, background: STATUS[v.status].bg }}>
                        {STATUS[v.status].label}
                      </span>
                    </td>
                    <td>
                      <button className="btn btn-gh" onClick={() => setDetail(v)} style={{ fontSize: 12, padding: "5px 12px" }}>
                        ดู →
                      </button>
                    </td>
                  </tr>
      ...
      {detail && (
        <div className="panel" onClick={e => e.target === e.currentTarget && setDetail(null)}>
          <div className="panel-box" style={{ maxWidth: 540 }}>
            <div className="panel-head">
              <div>
                <div style={{ fontWeight: 700, fontSize: 16 }}>รายละเอียดคำร้อง</div>
                <div style={{ fontSize: 11.5, color: "var(--t3)", fontFamily: "'DM Mono',monospace", marginTop: 3 }}>{detail.id}</div>
              </div>
              <button className="btn btn-gh" onClick={() => setDetail(null)} style={{ padding: "4px 10px", fontSize: 16 }}>✕</button>
            </div>
            <div className="panel-body" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {[
                ["👤 ผู้เข้าเยี่ยม",  detail.visitorName],
                ["🏢 บริษัท",          detail.company],
                ["🎯 วัตถุประสงค์",    detail.purpose],
                ["📅 วันที่เข้าพบ",    detail.visitDate],
                ["🕐 เวลา",            `${detail.enterTime}${detail.exitTime ? " – " + detail.exitTime : ""}`],
                ...
                ...(detail.approvedAt  ? [["🕐 อนุมัติ/ปฏิเสธเมื่อ", thaiDate(detail.approvedAt)]] : []),
              ].map(([l, v]) => (
                <div key={l} style={{ display: "flex", gap: 12 }}>
                  <div style={{ fontSize: 12, color: "var(--t3)", width: 180, flexShrink: 0 }}>{l}</div>
                  <div style={{ fontSize: 13, fontWeight: 500 }}>{v}</div>
                </div>
              ))}
              <div className="divider" style={{ margin: "4px 0" }} />
              <div style={{ display: "flex", justifyContent: "center" }}>
                <span className="badge" style={{ color: STATUS[detail.status].color, background: STATUS[detail.status].bg, fontSize: 13, padding: "6px 18px" }}>
...
fetch(GAS_URL, {
    method: 'POST',
    mode: 'no-cors', // GAS ไม่รองรับ CORS ต้องใช้ no-cors
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ action: 'submit', visitor }),
  });
  ...
  const handleApprove = (id, note) => {
    const next = visitors.map(v =>
      v.id === id ? { ...v, status: "approved", approveNote: note, approvedAt: now() } : v
    );
    setVisitors(next);
    saveVisitors(next);
fetch(GAS_URL, {
  ...
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({ action: 'approve', id, note }),
});
  ...
  const handleReject = (id, note) => {
    const next = visitors.map(v =>
      v.id === id ? { ...v, status: "rejected", approveNote: note, approvedAt: now() } : v
    );
    setVisitors(next);
    saveVisitors(next);
fetch(GAS_URL, {
      ...
      <div style={{ minHeight: "100vh" }}>
        ...
        <header style={{
          ...
          <div style={{
            ...
            display: "flex", alignItems: "center", gap: 6, height: 58,
          }}>
            <div style={{ display: "flex", alignItems: "center", gap: 9, marginRight: 14 }}>
              <div style={{
                width: 32, height: 32, borderRadius: 8, fontSize: 17,
                background: "linear-gradient(135deg,var(--ac),var(--ab))",
                display: "flex", alignItems: "center", justifyContent: "center",
              }}>🏭</div>
              <span style={{ fontWeight: 800, fontSize: 15, letterSpacing: "-.4px" }}>VisitorPass</span>
            </div>
            <nav style={{ display: "flex", gap: 4, overflowX: "auto" }}>
              {[
                { id: "form",    label: "📝 กรอกคำร้องขอ" },
                { id: "approve", label: `🔑 Approve${pending ? ` (${pending})` : ""}` },
        ...
        <footer style={{
          borderTop: "1px solid var(--br)", background: "var(--s1)",
          padding: "11px 24px", textAlign: "center", fontSize: 11.5, color: "var(--t3)",
        }}>
